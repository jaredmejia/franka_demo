...
if __name__ == "__main__":
    ...
    # Initialize
    print_and_cr(f"[INFO] Waiting to connect to Franka robot at {args.server_ip} ...")
    ...
    cmd_thread = Thread(target=keyboard_proc, name='Demo Keyboard Thread', args=(state,))
    cmd_thread.start()
    ...
    while not state.quit:
        ...
        if state.mode == 'teleop':
            ...
            print_and_cr(f"[INFO] Sending robot to home position. Takes about 4 sec. Do not move ..")
            state.franka.reset()
            ...
            joint_pos_desired = redis_receive_command(state.redis_store)
            print_and_cr(f"[DEBUG] Current state {robostate}")
            print_and_cr(f"[DEBUG] Received cmd {joint_pos_desired}")
            print_and_cr(f"[DEBUG] Diff {joint_pos_desired - robostate}")
            state.mode = 'idle'

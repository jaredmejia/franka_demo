...
height = 480
width = 640
frame_rate = 30 * 1 # 30 is camera FPS, 10 is realtime multiplier
...
for demo_folder in sorted(folders):
    if not os.path.isdir(demo_folder):
        continue
    video_save_folder = "/home/vdean/franka_demo/logs/videos"
    ...
    for trial in sorted(os.listdir(demo_folder)):
        trial = demo_folder # Victoria hack
        trial_path = os.path.join(demo_folder, trial)
        if not os.path.isdir(trial_path):
            print('trial path is not dir')
            continue

        reward = 1#, _ = get_reward(trial_path)
        if reward is None:
            bad_trial_counter += 1
            continue
